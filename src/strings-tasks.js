...
function sumOfCodes(str) {
  if (getStringLength(str) === 0) return 0;
  let res = 0;
...
function isPalindrome(str) {
  const s = str
    ...
    .join('');
  for (let a = 0, b = s.length - 1; a < s.length / 2; a += 1) {
...
function getStringFromTemplate(firstName, lastName) {
  return ['Hello,', firstName, lastName].join(' ').concat('!');
}
...
function extractNameFromTemplate(value) {
  return value.split(' ').slice(-2).join(' ').split('!')[0];
}
